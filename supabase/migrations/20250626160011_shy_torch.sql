/*
  ...
  1. Sample Data
    - Insert sample datasets with realistic business data
    - Insert sample KPIs with performance metrics
    - Insert sample dashboards with widget configurations
    - Insert sample portfolios with investment data

  2. Helper Functions
    - Create function to copy sample data for new users
    - Update user registration handler to include sample data
*/
...
BEGIN
    ...
    SELECT user_id, name, description, file_name, headers, row_count, column_count, data, NOW(), NOW()
    ...
    SELECT user_id, 'sample', name, description, formula, target_value, current_value, trend, category, NOW(), NOW()
    FROM sample_kpis
    ...
    SELECT user_id, name, description, layout, widgets, false, NOW(), NOW()
    ...
    SELECT user_id, name, description, total_value, performance, risk_score, assets, NOW(), NOW()
